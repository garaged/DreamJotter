...
public enum ScreenplayParser {
    public static func parse(_ source: String) -> ScreenplayDocument {
        ...
        while index < lines.count {
            ...
            if isInvalidSceneHeading(line) {
                ...
                diagnostics.append(.invalidSceneHeading(text: line))
                currentCharacter = nil
                index += 1
                continue
            }
            ...
            if line.hasPrefix("(") {
                if line.hasSuffix(")") {
                    ...
                    diagnostics.append(.malformedParenthetical(text: line))
                }
                continue
            }
            ...
            if let firstLine = paragraph.components(separatedBy: "\n").first,
               ...
               paragraph.contains("\n") {
                diagnostics.append(.ambiguousUppercaseLine(text: firstLine))
            }
    ...
    private static func consumeTitlePage(from lines: [String], index: inout Int) -> [String] {
        ...
        while index < lines.count {
            ...
            guard line.range(of: #"^[A-Za-z][A-Za-z ]*:"#, options: .regularExpression) != nil else {
                break
            }
    ...
    private static func scene(from heading: String) -> Scene {
        ...
        let location = parts.first?.trimmingCharacters(in: .whitespaces) ?? body
        let timeOfDay = parts.count > 1 ? parts.last?.trimmingCharacters(in: .whitespaces) : nil
    ...
    private static func isCharacterCue(_ line: String, in lines: [String], at index: Int) -> Bool {
        ...
        guard line.split(whereSeparator: \.isWhitespace).count <= 3 else {
            return false
        }

        guard let nextLine = nextNonEmptyLine(after: index, in: lines) else {
            return false
        }
...
private extension ScreenplayDiagnostic {
    static func ambiguousUppercaseLine(text: String) -> ScreenplayDiagnostic {
        ScreenplayDiagnostic(
            code: "ambiguousUppercaseLine",
            message: "Uppercase text was preserved without creating a character cue because dialogue context was not clear.",
            text: text
        )
    }

    static func invalidSceneHeading(text: String) -> ScreenplayDiagnostic {
        ScreenplayDiagnostic(
            code: "invalidSceneHeading",
            message: "Scene-like text is missing a supported scene heading prefix pattern.",
            text: text
        )
    }

    static func malformedParenthetical(text: String) -> ScreenplayDiagnostic {
        ScreenplayDiagnostic(
            code: "malformedParenthetical",
            message: "Parenthetical-like text is missing a closing parenthesis and must be preserved.",
            text: text
        )
    }
}
