...
| --- | --- | --- | --- | --- | --- | --- |
| M12-WRITER-WORKFLOW-POLISH | M12 | `docs/milestones/milestone-12-writer-workflow-polish.md` | `docs/acceptance/milestone-12-acceptance.md` | CommandEngine, ProjectDocumentViewModel, writer workspace views | slice-specific suites | implemented pending validation |
...
| M12-SCENE-WORKFLOW | M12.3 | `docs/specs/writer-workflow/m12-scene-workflow.spec.md` | `docs/acceptance/milestone-12-acceptance.md` | `Sources/DreamJotterCore/SceneWorkflow.swift`, `Apps/DreamJotterMac/Views/SceneWorkflowView.swift`, `Apps/DreamJotterMac/Views/BoundSceneWorkflowView.swift`, scene workflow view-model adapter | `Tests/DreamJotterExecutableSpecs/SceneWorkflowExecutableSpecs.swift` | implemented pending validation |

## Cross-Slice Traceability Rules

- Every high-impact mutation maps to a concrete command request and executable spec.
...
M12.2 coverage includes stored-note CRUD, Unicode-aware search, state and target filtering, parsed TODO separation, linked-target resolution and workspace navigation, resolve and reopen, snapshot failure blocking for bulk resolve, orphan detection and safe unlinking, dashboard/search projection updates, and package save/reopen persistence.

## M12.3 Coverage

M12.3 coverage includes editable scene metadata, Unicode-aware text and plotline filtering, planning-order separation, scene-card navigation, deterministic screenplay-order preview, snapshot-protected screenplay-order application, complete scene-block movement, snapshot failure handling, and package save/reopen persistence.
